...
public interface SpringDataMessageRepository extends JpaRepository<MessageJpaEntity, UUID> {
    ...
    List<MessageJpaEntity> findByChatRoomIdOrderBySentAtDesc(UUID chatRoomId, Pageable pageable);

    @Query("SELECT m FROM MessageJpaEntity m WHERE m.chatRoomId = :chatRoomId AND m.senderId <> :userId AND m.status <> 'READ'")
